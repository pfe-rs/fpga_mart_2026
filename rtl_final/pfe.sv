...
module convolution_3x3 #(
    ...
    int KERNEL[9] = '{-2, -1, 0, -1, 1, 1, 0, 1, 2};   
    int offsets[9];
    initial begin
        offsets[0] = -DIMENSION-1; offsets[1] = -DIMENSION; offsets[2] = -DIMENSION+1;
        offsets[3] = -1;           offsets[4] = 0;          offsets[5] = 1;
        offsets[6] =  DIMENSION-1; offsets[7] =  DIMENSION; offsets[8] =  DIMENSION+1;
    end
        ...
        if (!rst_ni) begin
            ...
            case (state)
                ...
                SUM: begin
                    ...
                    if (idx < 8) begin
                        idx       <= idx + 1;
                        read_addr <= center_ptr + offsets[idx + 1];
                        state     <= WAIT_DATA;
                    end else begin
                        state     <= FINISH;
                        out_valid_o <= 1'b1;
...
module pfe #(
    ...
    logic signed [DSIZE+7:0] full_res;
    ...
    assign out_data_o = (full_res > 16'sd255) ? 8'd255 : 
                    (full_res < 16'sd0)   ? 8'd0   : 
                    full_res[7:0];
endmodule