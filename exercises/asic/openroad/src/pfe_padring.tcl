# TODO: Potrebno nam je 10 pad-ova po stranici, ukoliko vam je potrebno manje ili vise, promenite ovaj broj
#32
set numPadsPerEdge 8
#x*4

# corner width is equal to padD, bondpad outside
set cornerToPad [expr {$padBond + $padD}]

make_io_sites -horizontal_site sg13g2_ioSite \
    -vertical_site sg13g2_ioSite \
    -corner_site sg13g2_ioSite \
    -offset $padBond \
    -rotation_horizontal R0 \
    -rotation_vertical R0 \
    -rotation_corner R0

###############################################################################
# Zadatak 6: Postavljanje I/O pinova
###############################################################################

##########################################################################
# Edge: LEFT (top to bottom)                                             #
##########################################################################
set westSpan  [expr {$chipH - 2*$cornerToPad - $padW}]
set westPitch [expr {floor($westSpan / double($numPadsPerEdge - 1))}]
set westStart [expr {$chipH - $cornerToPad - $padW}]

#ovo se menja, ima ih koliko i br
place_pad -row IO_WEST -location [expr {$westStart -  0*$westPitch}] "pad_vssio0"
place_pad -row IO_WEST -location [expr {$westStart -  1*$westPitch}] "pad_vddio0"
place_pad -row IO_WEST -location [expr {$westStart -  2*$westPitch}] "pad_HEX0_0"
place_pad -row IO_WEST -location [expr {$westStart -  3*$westPitch}] "pad_HEX0_1"
place_pad -row IO_WEST -location [expr {$westStart -  4*$westPitch}] "pad_HEX0_2"
place_pad -row IO_WEST -location [expr {$westStart -  5*$westPitch}] "pad_HEX0_3"
place_pad -row IO_WEST -location [expr {$westStart -  6*$westPitch}] "pad_vss0"
place_pad -row IO_WEST -location [expr {$westStart -  7*$westPitch}] "pad_vdd0"

##########################################################################
# Edge: BOTTOM (left to right)                                           #
##########################################################################
set southSpan  [expr {$chipW - 2*$cornerToPad - $padW}]
set southPitch [expr {floor($southSpan / double($numPadsPerEdge - 1))}]
set southStart $cornerToPad

place_pad -row IO_SOUTH -location [expr {$southStart +  0*$southPitch}] "pad_vssio1"
place_pad -row IO_SOUTH -location [expr {$southStart +  1*$southPitch}] "pad_vddio1"
place_pad -row IO_SOUTH -location [expr {$southStart +  2*$southPitch}] "pad_HEX0_4"
place_pad -row IO_SOUTH -location [expr {$southStart +  3*$southPitch}] "pad_HEX0_5"
place_pad -row IO_SOUTH -location [expr {$southStart +  4*$southPitch}] "pad_HEX0_6"
place_pad -row IO_SOUTH -location [expr {$southStart +  5*$southPitch}] "pad_rst_ni"
place_pad -row IO_SOUTH -location [expr {$southStart +  6*$southPitch}] "pad_vss1"
place_pad -row IO_SOUTH -location [expr {$southStart +  7*$southPitch}] "pad_vdd1"

##########################################################################
# Edge: RIGHT (bottom to top)                                            #
##########################################################################
set eastSpan  [expr {$chipH - 2*$cornerToPad - $padW}]
set eastPitch [expr {floor($eastSpan / double($numPadsPerEdge - 1))}]
set eastStart $cornerToPad


place_pad -row IO_EAST -location [expr {$eastStart +  0*$eastPitch}] "pad_vssio2"
place_pad -row IO_EAST -location [expr {$eastStart +  1*$eastPitch}] "pad_vddio2"
place_pad -row IO_EAST -location [expr {$eastStart +  2*$eastPitch}] "pad_ar"
place_pad -row IO_EAST -location [expr {$eastStart +  3*$eastPitch}] "pad_nsy"
place_pad -row IO_EAST -location [expr {$eastStart +  4*$eastPitch}] "pad_nsg"
place_pad -row IO_EAST -location [expr {$eastStart +  5*$eastPitch}] "pad_ewy"
place_pad -row IO_EAST -location [expr {$eastStart +  6*$eastPitch}] "pad_vss2"
place_pad -row IO_EAST -location [expr {$eastStart +  7*$eastPitch}] "pad_vdd2"

##########################################################################
# Edge: TOP (right to left)                                              #
##########################################################################
set northSpan  [expr {$chipW - 2*$cornerToPad - $padW}]
set northPitch [expr {floor($northSpan / double($numPadsPerEdge - 1))}]
set northStart [expr {$chipW - $cornerToPad - $padW}]

place_pad -row IO_NORTH -location [expr {$northStart -  0*$northPitch}] "pad_vssio3"
place_pad -row IO_NORTH -location [expr {$northStart -  1*$northPitch}] "pad_vddio3"
place_pad -row IO_NORTH -location [expr {$northStart -  2*$northPitch}] "pad_ewg"
place_pad -row IO_NORTH -location [expr {$northStart -  3*$northPitch}] "pad_pw"
place_pad -row IO_NORTH -location [expr {$northStart -  4*$northPitch}] "pad_clk_i"
place_pad -row IO_NORTH -location [expr {$northStart -  5*$northPitch}] "pad_unused0_o"
place_pad -row IO_NORTH -location [expr {$northStart -  6*$northPitch}] "pad_vss3"
place_pad -row IO_NORTH -location [expr {$northStart -  7*$northPitch}] "pad_vdd3"

###############################################################################
# Kraj zadatka 6: Postavljanje I/O pinova
###############################################################################
# Fill in the rest of the padring
place_corners $iocorner

place_io_fill -row IO_NORTH {*}$iofill
place_io_fill -row IO_SOUTH {*}$iofill
place_io_fill -row IO_WEST  {*}$iofill
place_io_fill -row IO_EAST  {*}$iofill

# Connect built-in power rings
connect_by_abutment

# Bondpad as separate cell placed in OpenROAD:
# place the bonding pad relative to the IO cell
place_bondpad -bond $bondPadCell -offset {5.0 -70.0} pad_*

# remove rows created by via make_io_sites as they are no longer needed
remove_io_rows
